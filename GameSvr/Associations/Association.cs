...
namespace GameSvr
{
    ...
    public class Association
    {
        ...
        public bool SaveGuildInfoFile()
        {
            ...
            {
                boChanged = true;
                M2Share.ErrorMessage($"保存行会[{sGuildName}]失败: {ex.Message}");
                return false;
            }
        }
        ...
        private void SaveGuildFile(string sFileName)
        {
            ...
            for (var i = 0; i < GuildAllList.Count; i++)
            {
                SaveList.Add("+" + GuildAllList[i].sGuildName);
            }
            SaveList.Add(" ");
            ...
            AtomicFile.WriteAllText(sFileName, SaveList.Text, HUtil32.GbkEncoding);
        }
        ...
        public void DelHumanObj(TPlayObject PlayObject)
        {
            ...
            for (var i = 0; i < m_RankList.Count; i++)
            {
                ...
                for (var j = 0; j < guildRank.MemberList.Count; j++)
                {
                    var member = guildRank.MemberList[j];
                    if (member != null && member.PlayObject == PlayObject)
                    {
                        member.PlayObject = null;
                        return;
                    }
                }
            }
        ...
        public void BackupGuildFile()
        {
            ...
            if (M2Share.nServerIndex == 0)
            {
                ...
                {
                    M2Share.ErrorMessage($"备份行会[{sGuildName}]失败: {ex.Message}");
                    return;
                }
            }
            for (var i = 0; i < m_RankList.Count; i++)
