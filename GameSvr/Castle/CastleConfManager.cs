...
namespace GameSvr
{
    public class CastleConfManager : IniFile
    {
        ...
        public void SaveConfig(TUserCastle userCastle)
        {
            ...
            {
                SetCachedString("Setup", "CastleName", userCastle.m_sName);
            }
            if (!string.IsNullOrEmpty(userCastle.m_sOwnGuild))
            {
                SetCachedString("Setup", "OwnGuild", userCastle.m_sOwnGuild);
            }
            SetCachedDateTime("Setup", "ChangeDate", userCastle.m_ChangeDate);
            SetCachedDateTime("Setup", "WarDate", userCastle.m_WarDate);
            SetCachedDateTime("Setup", "IncomeToday", userCastle.m_IncomeToday);
            if (userCastle.m_nTotalGold != 0) SetCachedInteger("Setup", "TotalGold", userCastle.m_nTotalGold);
            if (userCastle.m_nTodayIncome != 0) SetCachedInteger("Setup", "TodayIncome", userCastle.m_nTodayIncome);
            for (var i = 0; i < userCastle.m_EnvirList.Count; i++) sMapList = sMapList + userCastle.m_EnvirList[i] + ',';
            if (!string.IsNullOrEmpty(sMapList)) SetCachedString("Defense", "CastleMapList", sMapList);
            if (userCastle.m_sMapName != "") SetCachedString("Defense", "CastleMap", userCastle.m_sMapName);
            if (userCastle.m_sHomeMap != "") SetCachedString("Defense", "CastleHomeMap", userCastle.m_sHomeMap);
            if (userCastle.m_nHomeX != 0) SetCachedInteger("Defense", "CastleHomeX", userCastle.m_nHomeX);
            if (userCastle.m_nHomeY != 0) SetCachedInteger("Defense", "CastleHomeY", userCastle.m_nHomeY);
            if (userCastle.m_nWarRangeX != 0) SetCachedInteger("Defense", "CastleWarRangeX", userCastle.m_nWarRangeX);
            if (userCastle.m_nWarRangeY != 0) SetCachedInteger("Defense", "CastleWarRangeY", userCastle.m_nWarRangeY);
            if (userCastle.m_sPalaceMap != "") SetCachedString("Defense", "CastlePlaceMap", userCastle.m_sPalaceMap);
            if (userCastle.m_sSecretMap != "") SetCachedString("Defense", "CastleSecretMap", userCastle.m_sSecretMap);
            if (userCastle.m_nPalaceDoorX != 0) SetCachedInteger("Defense", "CastlePalaceDoorX", userCastle.m_nPalaceDoorX);
            if (userCastle.m_nPalaceDoorY != 0) SetCachedInteger("Defense", "CastlePalaceDoorY", userCastle.m_nPalaceDoorY);
            if (userCastle.m_MainDoor.nX != 0) SetCachedInteger("Defense", "MainDoorX", userCastle.m_MainDoor.nX);
            if (userCastle.m_MainDoor.nY != 0) SetCachedInteger("Defense", "MainDoorY", userCastle.m_MainDoor.nY);
            if (userCastle.m_MainDoor.sName != "") SetCachedString("Defense", "MainDoorName", userCastle.m_MainDoor.sName);
            if (userCastle.m_MainDoor.BaseObject != null)
            {
                SetCachedBool("Defense", "MainDoorOpen", userCastle.m_MainDoor.nStatus);
                SetCachedInteger("Defense", "MainDoorHP", userCastle.m_MainDoor.BaseObject.m_WAbil.HP);
            }
            if (userCastle.m_LeftWall.nX != 0) SetCachedInteger("Defense", "LeftWallX", userCastle.m_LeftWall.nX);
            if (userCastle.m_LeftWall.nY != 0) SetCachedInteger("Defense", "LeftWallY", userCastle.m_LeftWall.nY);
            if (userCastle.m_LeftWall.sName != "") SetCachedString("Defense", "LeftWallName", userCastle.m_LeftWall.sName);
            if (userCastle.m_LeftWall.BaseObject != null)
                SetCachedInteger("Defense", "LeftWallHP", userCastle.m_LeftWall.BaseObject.m_WAbil.HP);
            if (userCastle.m_CenterWall.nX != 0) SetCachedInteger("Defense", "CenterWallX", userCastle.m_CenterWall.nX);
            if (userCastle.m_CenterWall.nY != 0) SetCachedInteger("Defense", "CenterWallY", userCastle.m_CenterWall.nY);
            if (userCastle.m_CenterWall.sName != "") SetCachedString("Defense", "CenterWallName", userCastle.m_CenterWall.sName);
            if (userCastle.m_CenterWall.BaseObject != null)
                SetCachedInteger("Defense", "CenterWallHP", userCastle.m_CenterWall.BaseObject.m_WAbil.HP);
            if (userCastle.m_RightWall.nX != 0) SetCachedInteger("Defense", "RightWallX", userCastle.m_RightWall.nX);
            if (userCastle.m_RightWall.nY != 0) SetCachedInteger("Defense", "RightWallY", userCastle.m_RightWall.nY);
            if (userCastle.m_RightWall.sName != "") SetCachedString("Defense", "RightWallName", userCastle.m_RightWall.sName);
            if (userCastle.m_RightWall.BaseObject != null)
                SetCachedInteger("Defense", "RightWallHP", userCastle.m_RightWall.BaseObject.m_WAbil.HP);
            TObjUnit objUnit;
            for (var i = userCastle.m_Archer.GetLowerBound(0); i <= userCastle.m_Archer.GetUpperBound(0); i++)
            {
                objUnit = userCastle.m_Archer[i];
                if (objUnit.nX != 0) SetCachedInteger("Defense", "Archer_" + (i + 1) + "_X", objUnit.nX);
                if (objUnit.nY != 0) SetCachedInteger("Defense", "Archer_" + (i + 1) + "_Y", objUnit.nY);
                if (objUnit.sName != "")
                    SetCachedString("Defense", "Archer_" + (i + 1) + "_Name", objUnit.sName);
                if (objUnit.BaseObject != null)
                    SetCachedInteger("Defense", "Archer_" + (i + 1) + "_HP", objUnit.BaseObject.m_WAbil.HP);
                else
                    SetCachedInteger("Defense", "Archer_" + (i + 1) + "_HP", 0);
            }

            for (var i = userCastle.m_Guard.GetLowerBound(0); i <= userCastle.m_Guard.GetUpperBound(0); i++)
            {
                objUnit = userCastle.m_Guard[i];
                if (objUnit.nX != 0) SetCachedInteger("Defense", "Guard_" + (i + 1) + "_X", objUnit.nX);
                if (objUnit.nY != 0) SetCachedInteger("Defense", "Guard_" + (i + 1) + "_Y", objUnit.nY);
                if (objUnit.sName != "") SetCachedString("Defense", "Guard_" + (i + 1) + "_Name", objUnit.sName);
                if (objUnit.BaseObject != null)
                    SetCachedInteger("Defense", "Guard_" + (i + 1) + "_HP", objUnit.BaseObject.m_WAbil.HP);
                else
                    SetCachedInteger("Defense", "Guard_" + (i + 1) + "_HP", 0);
            }
            Save();
        }
    }
