    ...
    public partial class TPlayObject
    {
        ...
        private void ProcessBombItem(TUserItem item, GoodItem stdItem)
        {
            ...
            if (item.Dura == 0)
            {
                ...
                SendDefMessage(Grobal2.SM_DELITEM, item.MakeIndex, 0, 0, 0, "");
                return;
            }
            ...
            if (item.Dura > 0)
            {
                ...
                // 通知客户端更新耐久度
                // Notify client to update durability
                SendDefMessage(Grobal2.SM_UPDATEITEM, item.MakeIndex, 0, item.Dura, item.DuraMax, "");
            }
        ...
        private void ApplyBombDamage(int range, int damage)
        {
            ...
            foreach (var target in targets)
            {
                ...
                if (Math.Abs(target.m_nCurrX - m_nCurrX) > range ||
                    Math.Abs(target.m_nCurrY - m_nCurrY) > range)
                    continue;
