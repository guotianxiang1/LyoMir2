...
namespace GameSvr
{
    ...
    public partial class TPlayObject
    {
        ...
        {
            // 眼神 关闭摆摊 memcpys a single C3 over sub_6E7C38's prologue (0x100AD0EE
            // payload, 0x100AD113 push 0x6E7C38 / len 1 -> 0x10033340), so START
            // returns before the map gate, before the ladder and before any reply.
            // Bail ahead of GetOrCreate so no record is created either.
            // SetTimeLevel (4419 / sub_6E7938) is a different function and is not patched.
            if (op == NativeStallOp.StartStall
                && new Plugins.YanshenApi(this, null, M2Share.PluginManager).IsCloseStall())
                return true;
