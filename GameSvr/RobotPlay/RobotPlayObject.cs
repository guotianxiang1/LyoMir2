...
namespace GameSvr
{
    ...
    public partial class RobotPlayObject : TPlayObject
    {
        ...
        // 这里原本复制了一份 WinExp，连同基类那份一起带着五个非原生缩放
        // （nLimitExpLevel/nLimitExpValue 提前返回、dwKillMonExpMultiple、
        // m_nKillMonExpMultiple、m_nKillMonExpRate/100、地图旗标 EXPRATE、
        // m_rExpItem），全部在战神中零命中/不可达 —— 证据见
        // TPlayObject.NativeWinExp.cs §4。
        //
        // 战神只有一个 WinExp（sub_6F7A18），机器人与玩家共用同一段代码：
        // 机器人是 TPlayObject 的子类，基类那份忠实移植即可覆盖，重复一份
        // 只会让两边再次漂移。故此处的副本整体删除，不再 override。
        // 下面的 GetExp 同理只服务非击杀调用点。
        ...
        private bool SearchPickUpItem(int nPickUpTime)
        {
            ...
            try
            {
                ...
                if (this.IsEnoughBag() && m_TargetCret == null)
                {
                    ...
                    if (SelVisibleMapItem != null)
                    {
                        ...
                        {
                            WalkToTargetXY2(SelVisibleMapItem.nX, SelVisibleMapItem.nY);
                            result = true;
                        }
                    }
                    else
            ...
            {
                M2Share.MainOutMessage(" TAIPlayObject.SearchPickUpItem " + ex.Message);
            }
            return result;
        }
        ...
        private bool WalkToTargetXY2(int nTargetX, int nTargetY)
        {
            ...
            if (nTargetX != m_nCurrX || nTargetY != m_nCurrY)
            {
                if ((HUtil32.GetTickCount() - dwTick3F4) > m_dwTurnIntervalTime)// 转向间隔
                {
                    ...
                    if (!result)
                    {
                        n20 = M2Share.RandomNumber.Random(int.MaxValue);
        ...
        private bool CheckUserItemType(int nItemType, int nCount)
        {
            ...
            if (m_UseItems[Grobal2.U_ARMRINGL] != null && m_UseItems[Grobal2.U_ARMRINGL].wIndex > 0 &&
                Math.Round(Convert.ToDouble(m_UseItems[Grobal2.U_ARMRINGL].Dura / 100.0)) >= nCount)
            {
        ...
        private int GetUserItemList(int nItemType, int nCount)
        {
            ...
            for (var i = 0; i < m_ItemList.Count; i++)
            {
                ...
                if (StdItem != null)
                {
                    if (CheckItemType(nItemType, StdItem) && HUtil32.Round(UserItem.Dura / 100.0) >= nCount)
                    {
                        result = i;
                        break;
                    }
        ...
        private bool UseSpell(TUserMagic UserMagic, short nTargetX, short nTargetY, TBaseObject TargeTBaseObject)
        {
            ...
            switch (UserMagic.wMagIdx)
            {
                ...
                case SpellsDef.SKILL_MOOTEBO:
                    ...
                    {
                        TryStartNativeMotaeboForcedMove(UserMagic,
                            m_btDirection);
                    }
                    break;
        ...
        public bool Thinking()
        {
            ...
            {
                M2Share.MainOutMessage("TAIPlayObject.Thinking " + ex.Message);
            }
            return result;
        }
    ...
    m_PEnvir.MoveToMovingObject(m_nCurrX, m_nCurrY, this, m_nCurrX - 2, m_nCurrY - 2, true) > 0)
                ...
                if (m_UseItems[Grobal2.U_ARMRINGL].wIndex > 0)
                {
                    ...
                    if (AmuletStdItem != null)
                    {
                        if (AmuletStdItem.StdMode == 25)
                        {
                            ...
                            {
                                case 1:
                                    if (AmuletStdItem.Shape == 5 && Math.Round(Convert.ToDouble(m_UseItems[Grobal2.U_ARMRINGL].Dura / 100.0)) >= nCount)
                                    {
                                        result = true;
                                        return result;
                                    }
                                    break;
                                case 2:
                                    if (AmuletStdItem.Shape <= 2 && Math.Round(Convert.ToDouble(m_UseItems[Grobal2.U_ARMRINGL].Dura / 100.0)) >= nCount)
                                    {
                                        result = true;
                                        return result;
                                    }
                ...
                if (m_UseItems[Grobal2.U_BUJUK] != null && m_UseItems[Grobal2.U_BUJUK].wIndex > 0)
                {
                    ...
                    if (AmuletStdItem != null)
                    {
                        if (AmuletStdItem.StdMode == 25)
                        {
                            ...
                            {
                                case 1: 
                                    if (AmuletStdItem.Shape == 5 && Math.Round(Convert.ToDouble(m_UseItems[Grobal2.U_BUJUK].Dura / 100.0)) >= nCount)
                                    {
                                        result = true;
                                        return result;
                                    }
                                    break;
                                case 2: 
                                    if (AmuletStdItem.Shape <= 2 && Math.Round(Convert.ToDouble(m_UseItems[Grobal2.U_BUJUK].Dura / 100.0)) >= nCount)
                                    {
                                        result = true;
                                        return result;
                                    }
        ...
        private int GetDirBaseObjectsCount(int m_btDirection, int rang)
        {
            ...
            try
            {
                ...
                for (var step = 0; step <= rang; step++)
                {
                    MapCellinfo mapCellInfo = default;
                    if (!m_PEnvir.GetMapCellInfo(m_nCurrX + deltaX * step,
                            m_nCurrY + deltaY * step, ref mapCellInfo)
            ...
            catch (Exception)
            {
                M2Share.MainOutMessage(
                    "{异常} TBaseObject::GetDirectionBaseObjects");
            }
            return result;
        }
    }
