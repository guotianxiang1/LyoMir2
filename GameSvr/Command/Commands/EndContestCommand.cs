...
namespace GameSvr
{
    ...
    public class EndContestCommand : BaseCommond
    {
        ...
        {
            // 原版 @EndContest 命令名不在战神 430 行 GM 命令表(TABLE=0x7B4654, count=430, stride=0x78)中。
            ...
            // 表中赛事命令为 BeginAreaCastleMatch(id394)/EndAreaCastleMatch(id395)，'EndContest' 不在表中。
            // 证据: staging/idat_pass_C_refill_unkcmd_plumbing_20260803.md §Item D (D.1-D.5, Tier-1 反汇编);
            //       staging/ida_award_case584_command_registry_20260720.txt (430 行命令表内精确匹配 'EndContest' 为零命中)。
        }
    }
}
