...
namespace GameSvr
{
    ...
    public class ChangeSalveStatusCommand : BaseCommond
    {
        ...
        {
            // 眼神 plugin hook "禁止宝宝休息" (blocks turning rest ON inside length-15 map names).
            // Not part of the native arm; a pure predicate whose only effect is this early return,
            // so its position relative to the native gates below is unobservable except when it
            // and the DARE gate would both fire, in which case the player sees no message.
