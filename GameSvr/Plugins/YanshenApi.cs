...
namespace GameSvr.Plugins
{
    ...
    public class YanshenApi
    {
        ...
        // 了"7 级以下禁言"的等级阈值并硬编码返回，属凭空发明。
        //
        // 该删除已由插件字节复核（脱壳转储，基址 0x10000000）：配置键 "等级禁言"
        // (0x102B1688) 落在配置结构偏移 0x734（0x100B55C2 mov [ecx+0x734],eax），
        // 运行期只被读三次——0x1000A595、0x100CE930、0x100CE951——三处都是
        // cmp [.. +0x734],0x1F4 的开关判定，随后直接 call 0x100CE650 / 0x100CE710
        // 发提示串，全路径没有任何等级比较，也没有第二个参数字段。
        // 所以"等级 7"确实是发明，删对了。
        //
        // 但 S(1,1) 各模式（6/7/8）的实际效果仍不可判，理由要说准：不是"整个 DLL
        // 加壳"（.text 已脱开，插件读 S 的唯一入口 0x10056040 有 58 个调用点，
        // 逐个解出的 group/index 里根本没有 (1,1)），而是眼神 detour 了引擎 SetS
        // (0x100CEC39 装钩：0x6DF240 -> 0x100CEB40)，而 detour 本体被 Themida
        // 逐函数虚拟化——0x100CEB40 立即 push 密文块并 jmp 0x10C5FCD2，
        // 那 16 MB 在转储里全是零页。模式语义只存在于那段取不到的字节里。
        // 继续 fail-closed，不实现。开关本身仍可由 IsLevelMute() 读取。
