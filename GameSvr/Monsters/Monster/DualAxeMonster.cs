...
namespace GameSvr
{
    public class DualAxeMonster : Monster
    {
        ...
        private void FlyAxeAttack(TBaseObject Target)
        {
            if (m_PEnvir.CanFly(m_nCurrX, m_nCurrY, Target.m_nCurrX, Target.m_nCurrY))
            {
                ...
                if (nDamage > 0)
                {
                    Target.StruckDamage(nDamage, this);
        ...
        protected override bool AttackTarget()
        {
            ...
            if ((HUtil32.GetTickCount() - m_dwHitTick) > m_nNextHitTime)
            {
                ...
                if (Math.Abs(m_nCurrX - m_TargetCret.m_nCurrX) <= 7 && Math.Abs(m_nCurrY - m_TargetCret.m_nCurrY) <= 7)
                {
                ...
                {
                    if (Math.Abs(m_nCurrX - m_TargetCret.m_nCurrX) <= 11 && Math.Abs(m_nCurrY - m_TargetCret.m_nCurrY) <= 11)
                    {
        ...
        public override void Run()
        {
            ...
            if (!m_boDeath && !m_boGhost && m_wStatusTimeArr[Grobal2.POISON_STONE] == 0)
            {
                ...
                {
                    if (Math.Abs(m_nCurrX - m_TargetCret.m_nCurrX) <= 4 && Math.Abs(m_nCurrY - m_TargetCret.m_nCurrY) <= 4)
                    {
