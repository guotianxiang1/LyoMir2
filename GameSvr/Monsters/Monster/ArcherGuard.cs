...
namespace GameSvr
{
    public class ArcherGuard : GuardUnit
    {
        public ArcherGuard() : base()
        {
            ...
            m_boWantRefMsg = true;
            m_Castle = null;
            // 0x684819 `mov dword [esi+0x4E0], -1` -- the ONLY immediate write to
            // this field anywhere in the image. -1 means "no idle facing", and
            // 0x6848A8's signed `jl` then skips the idle turn entirely. Writing 0
            // here would make the archer snap to direction 0 on its first idle
            // tick instead of holding its placed facing.
            m_nDirection = -1;
        }
        ...
        private void sub_4A6B30(TBaseObject TargeTBaseObject)
        {
            ...
            if (nPower > 0)
            {
                ...
                TargeTBaseObject.StruckDamage(nPower, this);
        ...
        public override void Run()
        {
            ...
            if (!m_boDeath && !m_boGhost && m_wStatusTimeArr[Grobal2.POISON_STONE] == 0)
            {
                ...
                {
                    // 0x6848A0 mov eax,[ebx+0x4E0] / 0x6848A6 test eax,eax /
                    // 0x6848A8 jl -> skip. The `jl` is SIGNED, so the -1 sentinel
                    // means "hold whatever facing you have". Then 0x6848AC compares
                    // the byte m_btDirection (+0x154) and 0x6848BE turns.
                    if (m_nDirection >= 0 && m_btDirection != m_nDirection)
                    {
                        TurnTo((byte)m_nDirection);
                    }
                }
            }
            base.Run();
