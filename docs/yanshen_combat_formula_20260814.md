...
| **F-5** | 切割 `lei==1` 的 **8 向朝向筛格** | 8 个分支（`1006EE07`..`1006EF22`）语义已读出，但每支的边界符号（`jns` / `js` / `jne`）组合复杂，且依赖 `byte[caster+0x154]` 的方向编码与本仓 `DR_*` 的对齐关系未逐值验证 | 已实现"以施法者为中心"这一半；筛格未做（等价于 `lei==1` 时打满方框，**打得比原生多**）。要修需先把 `DR_*` 与 `+0x154` 的 8 个取值逐值对齐 |
